...
#if TPD_AUTO_UPGRADE
/*lenovo project*/
static unsigned char CTPM_FW1[]=          //oufei   FHD
{
#include "ft_app_ic_v12_ofilm.txt"
};

static unsigned char CTPM_FW2[]=          //shenyue  HD
{
  #include "ft_app_ic_v12_mutto.txt"
};

static unsigned char CTPM_FW3[]=          //shenyue   FHD
{
  #include "ft_app_ic_v12_shengyue_FHD.txt"
};


static unsigned char CTPM_FW4[]=           //oufei HD
{
  #include "ft_app_ic_v12_oufei_HD.txt"
};
...
int fts_ctpm_fw_upgrade_with_i_file(struct ft5x06_ts_data *data)
{
	...
	//CTP_DEBUG("vendor_id:%d", vendor_id);
        flag_TPID = 1;
    }
	
    if(vendor_id == VENDOR_O_FILM)                                //oufei   FHD
    {
        pbt_buf = CTPM_FW1;
        fw_len = sizeof(CTPM_FW1);
        CTP_DEBUG("update firmware size:%d", fw_len);
    }
    else if(vendor_id == VENDOR_O_SHENYUE)               //Shenyue  HD
    {
        pbt_buf = CTPM_FW2;
        fw_len = sizeof(CTPM_FW2);
        CTP_DEBUG("update firmware size:%d", fw_len);
    }
   else if(vendor_id == VENDOR_O_SHENYUE_FHD)             //Shenyue FHD
    {
        pbt_buf = CTPM_FW3;
        fw_len = sizeof(CTPM_FW3);
        CTP_DEBUG("update firmware size:%d", fw_len);
    }
    else if(vendor_id == VENDOR_O_FILM_HD)                    //oufei HD
    {
        pbt_buf = CTPM_FW4;
    ...
    if (sizeof(CTPM_FW1) < 8 || sizeof(CTPM_FW1) > 32 * 1024 || sizeof(CTPM_FW2) < 8 || sizeof(CTPM_FW2) > 32 * 1024 ||sizeof(CTPM_FW3) < 8 || sizeof(CTPM_FW3) > 32 * 1024|| sizeof(CTPM_FW4) < 8 || sizeof(CTPM_FW4) > 32 * 1024)
    {
...
static ssize_t ctp_openshort_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
    ...
    *ppos += count;
    ...
    if(fw_info[5] == VENDOR_O_FILM)                            //FHD oufei
    {
        sprintf(cfgname, "%s", "ft5x06_ic_v14_ofilm.ini");
        CTP_INFO("ic is oufei FHD");
    }
    else if(fw_info[5] == VENDOR_O_SHENYUE)           //HD Shenyue   
    {
        sprintf(cfgname, "%s", "ft5x06_ic_v14_shenyue.ini");
        CTP_INFO("ic is shenyue HD");
    }
   else if(fw_info[5] == VENDOR_O_SHENYUE_FHD)          //FHD Shenyue
   {
        sprintf(cfgname, "%s", "ft5x06_ic_shenyue_FHD.ini");
        CTP_INFO("ic is shenyue FHD");
   }
   else if(fw_info[5] == VENDOR_O_FILM_HD)                   //HD oufei
   {
        sprintf(cfgname, "%s", "ft5x06_ic_oufe_HD.ini");
        CTP_INFO("ic is oufei HD");
   }
  else
    {
...
static int hardwareinfo_set(struct ft5x06_ts_data *data,u8 value_name)
{
	...
	if (data->fw_vendor_id == VENDOR_O_FILM)                                                      //FHD oufei
		snprintf(vendor_for_id,HARDWARE_MAX_ITEM_LONGTH,"Oufei_ FHD");
	else if (data->fw_vendor_id == VENDOR_O_SHENYUE)
		snprintf(vendor_for_id,HARDWARE_MAX_ITEM_LONGTH,"Shenyue_HD");    //HD shenyue
	else if (data->fw_vendor_id == VENDOR_O_FILM_HD)
		snprintf(vendor_for_id,HARDWARE_MAX_ITEM_LONGTH,"Oufei_HD");         //HD oufei
        else if (data->fw_vendor_id == VENDOR_O_SHENYUE_FHD)
		snprintf(vendor_for_id,HARDWARE_MAX_ITEM_LONGTH,"Shenyue_FHD");           //FHD shenyue
	else 
...
/*
    ...
    dev_info(&client->dev, "report rate = %dHz\n", reg_value * 10);                  //delete redundant info
